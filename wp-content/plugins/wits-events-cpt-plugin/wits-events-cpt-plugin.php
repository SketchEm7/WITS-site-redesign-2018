...
/*
 * @package WITS Events Custom Post Type Plugin
*/
/*
  Plugin Name: WITS Events Custom Post Type Plugin
  Plugin URI: https://womenintechsummit.net/
  Description: A plugin that creates one main custom post type, to create other custom post types for our various events
  Version: 1.0.0
  Author: Emily Melendez
  Author URI: https://womenintechsummit.net/
  License: GPLv2 or later
  Text Domain: wits-events-cpt-plugin
*/

/*HOOKS*/
add_action('save_post', 'cpt_save_postdata');
add_action('add_meta_boxes', 'cpt_add_meta_boxes');
add_action('init', 'register_main_event_cpt');

function register_main_event_cpt(){
	$labels = array(
		'name' => 'Events',
		'singular_name' => 'Event',
	...
	$the_query = new WP_Query(array('post_type' => array('Event')));
...
function cpt_add_meta_boxes() {
    add_meta_box('cpt_meta_id', 'Custom Post Type Settings', 'cpt_inner_custom_box', 'Event', 'normal');
